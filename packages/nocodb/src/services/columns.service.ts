...
export class ColumnsService implements IColumnsService {
  ...
  private async simpleColumnUpdate(
    ...
    // Mirrors the diff in _runColumnUpdate — an unchanged title is not a rename.
    const titleChanged =
      'title' in param.column && param.column.title?.trim() !== column.title;
    ...
    // Nothing differs (e.g. a title-only PATCH with the current title) — skip
    // the write instead of bumping updated_at and dropping caches for a no-op.
    if (Object.values(updateObj).some((v) => v !== undefined)) {
  ...
  async columnUpdate(
    ...
    // Plan/feature gates (EE) — pre-persist, runs on every route and runtime
    // state (including unlicensed on-prem, which bypasses the EE method body).
    await this.preColumnUpdate(context, param, column);
  ...
  protected async _runColumnUpdate(
    ...
    // A prop submitted with the value it already has is a no-op, not a
    // modification, so the guards below must not fire on it. Only title and
    // column_name are diffed — everything else stays conservatively counted as
    // a change (booleans arrive as 0/1 from sqlite, meta is a JSON string).
    const submittedPropIsUnchanged = (k: string) => {
      if (k === 'title') return param.column.title?.trim() === column.title;
      if (k === 'column_name')
        return param.column.column_name === column.column_name;
      return false;
    };

    const payloadHasNonMetaProps = Object.keys(param.column).some(
      (k) => !META_ONLY_COLUMN_PROPS.has(k) && !submittedPropIsUnchanged(k),
    );
    ...
    // Omitting description means "leave unchanged" — without the `in` check an
    // absent key reads as undefined !== null and pushes every meta-only payload
    // through the full Column.update() path.
    if (
      'description' in param.column &&
      param.column.description !== column.description
    ) {
    ...
    if (
      ...
      if (colBody.uidt === UITypes.AutoNumber) {
        ...
        isAutoGeneratedColumn(column) &&
        colBody.uidt !== column.uidt
      ) {
        // Converting away from AutoNumber/UUID: the value is no longer
        // DB-generated, so the column must become writable again. Gated on an
        // actual type change — a plain rename of a UUID column keeps readonly.
        colBody.readonly = false;
      }
      ...
      setPropsFromRequest('cdf');

      // Nullability is system-owned, never client-settable — columnAdd strips
      // `rqd` via deleteColumnSystemPropsFromRequest, so accepting it here let
      // callers reach a NOT NULL column with no default that nothing populates.
      // Deleting it makes updateMetaAndDatabase fall back to the stored value,
      // preserving whatever the physical schema already has.
      delete colBody.rqd;
  ...
  async columnAdd<T extends NcApiVersion = NcApiVersion | null | undefined>(
    ...
    // Plan/feature gates (EE) — pre-persist so unavailable field types are
    // rejected before any write, on every route (v2 columns + v3 fields).
    await this.preColumnAdd(context, param);
  ...
  // Pre-persist hook invoked at the top of `columnAdd`, before any validation
  // that writes. EE overrides this to run plan/feature gates so they fire in
  // every runtime state — including unlicensed on-prem, where `@EEOnly()`
  // bypasses the EE `columnAdd` body but still routes through this CE base.
  protected async preColumnAdd(
    _context: NcContext,
    _param: {
      tableId: string;
      column: ColumnReqType;
    ...
    // placeholder for pre column add hook
  }

  // Pre-persist counterpart of `preColumnAdd` for `columnUpdate`. Receives the
  // already-loaded current column so EE gates can compare old vs new state.
  protected async preColumnUpdate(
    _context: NcContext,
    _param: {
      columnId: string;
      column: ColumnReqType & { colOptions?: any };
    ...
    _currentColumn: Column<any>,
  ) {
    // placeholder for pre column update hook
  }
