...
const up = async (knex: Knex) => {
  ...
  await knex.schema.createTable(MetaTable.PROJECT, (table) => {
    ...
    table
      .specificType('version', 'smallint')
      .unsigned()
      .defaultTo(BaseVersion.V2);
  ...
  await knex.schema.createTable(MetaTable.DB_SERVERS, (table) => {
    ...
    table.integer('max_tenant_count');
  ...
  await knex.schema.createTable(MetaTable.MODEL_STAT, (table) => {
    ...
    table.string('base_id', 20);
    table.timestamps(true, true);
    table.primary(['fk_workspace_id', 'base_id', 'fk_model_id']);
  });
...
const down = async (knex: Knex) => {
  ...
  await knex.schema.dropTableIfExists(MetaTable.DB_SERVERS);
  ...
  await knex.schema.dropTableIfExists(MetaTable.WIDGETS);
  await knex.schema.dropTableIfExists(MetaTable.DASHBOARDS);
  await knex.schema.dropTableIfExists(MetaTable.ROW_COLOR_CONDITIONS);
};
