...
import { UITypes, ViewTypes, isAIPromptCol, isAutoGeneratedColumn, isLinksOrLTAR, isSystemColumn, isVirtualCol, PermissionEntity, PermissionKey } from 'nocodb-sdk'
...
async function clearCell(ctx: { row: number; col: number } | null, skipUpdate = false) {
  ...
  if (colMeta.value[ctx.col].isReadonly && !isVirtualCol(fields.value[ctx.col])) {
    if (isAutoGeneratedColumn(fields.value[ctx.col])) {
...
const colMeta = computed(() => {
  return fields.value.map((col) => {
    return {
      ...
      isReadonly: isReadonlyVirtualColumn(col) || isAutoGeneratedColumn(col) || col.readonly,
    }
  })
})
...
async function clearSelectedRangeOfCells() {
  ...
  for (const row of rows) {
    for (const col of cols) {
      ...
      if (isReadonlyVirtualColumn(col) || isAutoGeneratedColumn(col) || col.readonly) continue
