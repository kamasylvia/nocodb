...
> 审查员: f09r10l4 | session: f09r10l4 | 基线: 5bea0c3943 (HEAD, 无代码变更)
> 日期: 2026-09-18 | 后端: :8080 | 前端: :3000 Nuxt dev
> 测试数据: f09r10l4-* 前缀（src base / dest base / syncs，测完全删）
...
以 admin token（org-level-creator + super）实测：

| 端点 | 预期 | 实际 | 判定 |
|---|---|---|---|
| listSyncs | 200 | 200 | PASS |
| sourceSchema | 200 | 200 | PASS |
...
- **full-create (delete 策略)**: sync 创建后 3s → status=active；mirror 表 3 行 = 源 3 行；RemoteId 非空（1/2/3）
- **系统列元数据**: RemoteId `system=True, readonly=True, meta.defaultViewColVisibility=False`；RemoteDeleted 同——双保险确认
- **freeze/resume**: freeze → paused → resume → active（状态翻转正确）
- **resync**: resync → active（upsert 完成）
- **mark_deleted 策略**: 同流程创建 → active
- **deleteSync**: DELETE → 200，sync 从 list 消失
...
Grid view 列元数据确认：
- `ckj3srbrsas06zh`（RemoteId）: show=False
- `czj1uqixcocjj6w`（RemoteDeleted）: show=False

其余 8 列（含引擎系统列 CreatedAt/UpdatedAt 等）均 show=True。RemoteId/RemoteDeleted 双保险（system=true + show=false）确认隐藏。

### A9. E1 六格矩阵 — PASS

m3/m4（私有 base + 无 base role）返回 404——比预期的 403 更严格，属 fail-closed 方向，判 PASS（R10 附录 A.1 方法学注记：403/404 双 fail-closed 码均 PASS）。

### A10. R5 四象限 — PASS

q1（非私有 + ws-creator）→ 200；q3（viewer on dest）→ 403。符合预期。
...
- 未使用 psql。
- camoufox session f09r10l4 已关闭。
- 测试数据全 `f09r10l4-` 前缀；src base + dest base + syncs 测完删除，列表核实 residual: NONE。
