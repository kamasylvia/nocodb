你是 NocoDB CE-EE fork 的 F03 Data permissions 第 6 轮复审（R6）独立审查员。工作目录：/Volumes/UNITEK/Documents/Development/nocodb。
...
- 审查对象 HEAD 前移：b28787a54a 之后增加 i18n 修复提交（2 个弹窗的 selectUsers 键路径）。
- **回归新增强**：`labels.selectUsers` 裸键已修为 `objects.permissions.inlineUserSelector.selectUsers`——验证 Table/Field 两弹窗空选 Save 的报错 toast 文案正常（非原始键）。
- **已知非问题（勿再报）**：①成员下拉"8 条截断"已实测驳回——users 端点返回全量（dev 库 739 用户全部在列），lane5 旧观察系脏库噪声；②dev 库 700+ 测试账号为环境噪声；③v1 bulkUpsert 500 / v1 title 寻表 404 / sharedView meta 不消费 VISIBILITY / duplicate >1000 行 job 失败 / duplicate 不带成员角色 = 上游 E3/backlog。
- **测试隔离纪律**：历史轮发生过并发路批量降权他路账号的事故——只创建/修改/删除**自己** lane 前缀的账号与数据，禁止触及任何非本 lane 前缀的账号。

## R6 专属附录（R5 修复批回归，commit ca8bb77622）

- **回归新增强**（4 项全要实测）：
  1. Details Permissions tab 摘要：无 grant 时 ADD/DELETE 应显示 "Editors & up"（不再 Everyone）；VISIBILITY 无 grant 仍 "Everyone"
  2. Table 弹窗 VISIBILITY 组出现 Creators & up 选项；API 造 role:owner VISIBILITY grant 后弹窗回显选中该档，保存不降权
  3. POST/PATCH nobody grant 且显式携带 granted_role → 400（create+update 对称）；不带 role 的 nobody 往返仍 200
  4. permissions.service.ts F03 TABLE 校验块已有 [CE-EE] 标记（代码审）
- **已知非问题/上游 E3 沿袭 R5 清单**，另加：duplicate >1000 行 job 失败（上游 chunk）
- **测试隔离纪律**沿袭：只动自己 f06lN-* 前缀数据；**严禁 psql 提全局 super**（会被 owner 直通造成 enforcement 假放行）
