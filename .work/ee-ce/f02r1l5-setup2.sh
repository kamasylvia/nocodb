...
BASE=$(curl -s -X POST $API/api/v2/meta/bases -H "xc-auth: $OT" -H 'Content-Type: application/json' -d '{"name":"f02r1l5"}' | jqpy "d['id']")
...
LNK=$(curl -s -X POST $API/api/v2/meta/tables/$T1/columns -H "xc-auth: $OT" -H 'Content-Type: application/json' -d "{\"uidt\":\"Links\",\"title\":\"Lnk\",\"column_name\":\"lnk\",\"parentId\":\"$T1\",\"childId\":\"$T2\",\"type\":\"mm\"}")
LNK_ID=$(echo "$LNK" | jqpy "d['id']")
...
echo "t1=$T1 secret=$SECRET_ID lnk=$LNK_ID"

curl -s -X POST $API/api/v2/tables/$T1/records -H "xc-auth: $OT" -H 'Content-Type: application/json' -d '{"Title":"r1","Secret":"s1"}' | head -c 120; echo
curl -s -X POST $API/api/v2/tables/$T2/records -H "xc-auth: $OT" -H 'Content-Type: application/json' -d '{"Title":"c1"}' >/dev/null

# invite editor into base (v2 route)
